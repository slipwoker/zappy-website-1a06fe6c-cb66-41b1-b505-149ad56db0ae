  ...
  if (contactForm) {
    contactForm.addEventListener('submit', function(e) {
      ...
      if (!name || !email || !phone || !consent) {
        alert('אנא מלאו את כל השדות הנדרשים ואשרו את תנאי השימוש');
        return;
      }
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "בייקידיס העצמה פוגשת אפייה משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
                ...
                body: JSON.stringify({
                    websiteId: '1a06fe6c-cb66-41b1-b505-149ad56db0ae',
                    name: data.name || '',
                    email: data.email || '',
